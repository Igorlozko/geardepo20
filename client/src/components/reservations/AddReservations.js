import { AppBar, Box, Button, Container, Dialog, Divider, IconButton, Slide, Stack, Step, StepButton, Stepper, Toolbar, Typography } from '@mui/material'
...
const AddReservations = () => {
    const {state:{resDetails, dateRange, currentUser,reservation},dispatch} = useValue();
    ...
    const handleClose = ()=>{
        dispatch({type:'UPDATE_RESERVATION', payload: null})
    ...
    // how it works when i close the reservation dialog the handleclose function updates the reservation to null closing the dialog , if the reservation is set to null the dialog wont open again when clicked 
  return (
    // container centeralises compnments within the page 
    <Dialog
        fullScreen
        open={Boolean(reservation !== null)} // check if the reservation is not null
        onClose={handleClose}
